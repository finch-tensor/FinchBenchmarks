...
ALL_TARGETS = $(SPADD_TACO) $(SPMV_TACO) $(SPGEMM_TACO) 
...
$(TACO): $(TACO_CLONE)
	...
	mkdir -p build ;\
	cd build ;\
	cmake -DPYTHON=false -DCMAKE_BUILD_TYPE=Release -DOPENMP=ON .. ;\
	make taco -j$(NPROC_VAL) ;\
	touch hello

spadd/spadd_taco: $(SPARSE_BENCH) $(TACO) spadd/spadd_taco.cpp
	$(CXX) $(CXXFLAGS) $(TACO_CXXFLAGS) -o $@ spadd/spadd_taco.cpp $(LDLIBS) $(TACO_LDLIBS)
