...
records = []
for path in glob.glob("./hist_*_threads.json"):
    with open(path) as f:
        records.extend(json.load(f))

# Build dict: matrix -> {threads -> time_ms}  (coalesce_impl only)
data: dict = {}
...
ax.set_title("Structured Histogram Strong Scaling", fontweight="bold", pad=14)
...
ax.legend(loc="upper right", bbox_to_anchor=(1.0, 1.04), framealpha=0.85, edgecolor="#cccccc", ncol=2)

plt.tight_layout()
plt.savefig("./hist_strong_scaling.png", dpi=200)
print("Saved.")
