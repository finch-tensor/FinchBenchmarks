...
GUSTAVSON_VARIANTS = {"spgemm_finch_gustavson_static", "spgemm_finch_gustavson_dynamic"}
GUSTAVSON_MERGED   = "spgemm_finch_gustavson"
...
def main(filename):
    ...
    for entry in data:
        ...
        # Merge static and dynamic gustavson by taking the minimum time
        if method in GUSTAVSON_VARIANTS:
            existing = times_by_matrix[matrix].get(GUSTAVSON_MERGED)
            times_by_matrix[matrix][GUSTAVSON_MERGED] = min(time, existing) if existing is not None else time
        else:
            times_by_matrix[matrix][method] = time

    baseline_method = 'spgemm_mkl'
