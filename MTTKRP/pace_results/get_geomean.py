...
def main(filename):
    ...
    # Add synthetic coalesce entry as min of static and dynamic
    for matrix, times in times_by_matrix.items():
        static = times.get('coalesce_static')
        dynamic = times.get('coalesce_dynamic')
        if static is not None and dynamic is not None:
            times['coalesce'] = min(static, dynamic)

    baseline_method = 'taco_impl'
