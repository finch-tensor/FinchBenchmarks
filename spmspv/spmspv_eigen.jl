...
function spgemm_eigen(A, B)
    ...
    fwrite(B_path, Tensor(SparseList(Element(0.0)), B)) #TACO matrix market readerr can only read real-valued matrices
    spmspv_path = joinpath(@__DIR__, "spmspv_eigen")
    withenv() do
        run(`$spmspv_path -i $tmpdir -o $tmpdir`)
    end 
    C = fread(C_path)
